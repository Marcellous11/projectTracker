...
/**
 * Merged activity stream — commits + STATUS.md mtime markers,
 * one chronological feed per request. Powers the
 * Activity Heatmap and the per-project Timeline.
 *
...
/** @typedef {{
 *   ts: string,
 *   type: "commit"|"status"|"quake"|"hn"|"infra"|"space",
...
export const getActivityFeed = cache(async (opts = {}) => {
  ...
  let all = [...commits, ...status, ...external]
...
export const getDailyActivity = cache(async (days = 84) => {
  ...
  for (let i = 0; i < days; i++) {
    ...
    buckets.set(dayKey(d), { date: dayKey(d), count: 0, commits: 0 });
  }
