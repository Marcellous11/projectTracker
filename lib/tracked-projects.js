...
export const getTrackedProjects = cache(async (root = projectsRoot()) => {
  ...
  for (const entry of config) {
    ...
    // ~2 min — trust it for recency: take the newer of the two dates, and
    // refresh status from live activity.
    //
    // STATUS.md's `status`/`last_worked` are hand-maintained and rot the moment
    // you push from another machine without editing them — a repo reads
    // "blocked · 40d" long after you've shipped. So when GitHub activity is
    // clearly newer than the manual `last_worked` (by more than a day), the
    // manual status is stale and live activity wins — even over "blocked"/"done".
    // A manual status you set *after* your last push (still newer than GitHub)
    // is respected, so a fresh, deliberate "blocked" isn't clobbered.
    const DAY_MS = 86400000;
    const ghIso = gh?.pushedAt || gh?.lastCommit?.date || null;
    const ghDays = daysSince(ghIso);
    const manualLastWorked = base.lastWorked ? new Date(base.lastWorked) : null;
    const ghDate = ghIso ? new Date(ghIso) : null;
    const manualStatusStale =
      ghDate &&
      !isNaN(ghDate) &&
      manualLastWorked &&
      !isNaN(manualLastWorked) &&
      ghDate.getTime() - manualLastWorked.getTime() > DAY_MS;
    if (ghDays != null && (base.staleDays == null || ghDays < base.staleDays)) {
      base.staleDays = ghDays;
      base.lastWorked = ghDate;
      if (
        manualStatusStale ||
        ["active", "paused", "untracked"].includes(base.status)
      ) {
