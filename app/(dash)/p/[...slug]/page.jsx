...
import {
  ...
  Worklist,
  RecentlyDone,
} from "@/components/project/status-sections.jsx";
...
export default async function BriefingPage({ params }) {
  ...
  // GitHub snapshot for this local project (keyed by rel).
  ...
  // Last 5 commits from the local git history, normalized to the shared shape.
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {/* The numbers that matter, up top. */}
      <StatStrip detail={detail} prCount={prCount} itineraryCount={itinerary.length} />

      {/* Where we are now + any blockers. */}
      <WhereWeAre nextAction={detail.nextAction} blockers={detail.blockers} />

      {/* What's left — the To-do / acceptance-criteria checklist. */}
      <Worklist todos={detail.todos} counts={detail.todoCounts} />
      ...
      {/* Full STATUS.md / README — supplementary, raw reference. */}
